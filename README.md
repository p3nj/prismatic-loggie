...
- **Schedule editing** — schedule type dropdown (None / Every minute / Hourly / Daily / Weekly / Custom), always-visible cron input, and an IANA timezone autocomplete backed by the browser's `Intl.supportedValuesOf('timeZone')`. Saving is blocked if a non-None schedule has an empty or malformed cron.
...
- **JSON view** — a read-only JSON tab surfaces connection inputs (secret values masked), schedule cron + timezone + scheduleType, and connection status / refresh metadata. Editing is done on the Variables tab. Previously this tab showed `null` for connection and schedule values.
